"use strict";

var dropDown = document.getElementById("user_type");
var userFields = document.getElementById("user_fields");
var adminFields = document.getElementById("admin_fields");
var normalUserName = document.getElementById("normalUserName");
var adminUserName = document.getElementById("adminUserName");
var secretPass = document.getElementById("secretPass");
var enterBtn = document.getElementById("enterBtn");
var form = document.querySelector("form");
var togglePassword = document.getElementById("togglePassword");

function checkUserType() {
  var val = dropDown.value;

  if (val == "normal") {
    userFields.style.display = "flex";
    adminFields.style.display = "none";
    normalUserName.setAttribute("required", "true");
    adminUserName.removeAttribute("required");
    secretPass.removeAttribute("required");
  } else {
    adminFields.style.display = "flex";
    userFields.style.display = "none";
    adminUserName.setAttribute("required", "true");
    secretPass.setAttribute("required", "true");
    normalUserName.removeAttribute("required");
  }
}

dropDown.onchange = checkUserType;
togglePassword.addEventListener("click", function () {
  var type = secretPass.getAttribute("type") === "password" ? "text" : "password";
  secretPass.setAttribute("type", type); // تغيير الأيقونة

  var eyeIcon = this.querySelector("i");
  eyeIcon.classList.toggle("fa-eye");
  eyeIcon.classList.toggle("fa-eye-slash");
}); // دوال حفظ واسترجاع بيانات المستخدم

function saveUserData(userType, username) {
  if (document.getElementById('remember').checked) {
    localStorage.setItem('userType', userType);
    localStorage.setItem('username', username);
    localStorage.setItem('remembered', 'true');
  } else {
    localStorage.removeItem('userType');
    localStorage.removeItem('username');
    localStorage.removeItem('remembered');
  }
}

function loadUserData() {
  var remembered = localStorage.getItem('remembered');

  if (remembered) {
    var userType = localStorage.getItem('userType');
    var username = localStorage.getItem('username'); // تعيين نوع المستخدم

    dropDown.value = userType;
    checkUserType(); // تعيين اسم المستخدم

    if (userType === 'normal') {
      normalUserName.value = username;
    } else {
      adminUserName.value = username;
    } // تحديد checkbox


    document.getElementById('remember').checked = true;
  }
} // دالة لإظهار رسائل


function showToast(type, title, message) {
  var duration = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 5000;
  var container = document.getElementById('toast-container');
  if (!container) return; // التحقق من وجود العنصر
  // إنشاء عنصر الرسالة

  var toast = document.createElement('div');
  toast.className = "toast ".concat(type); // إضافة الأيقونة حسب نوع الرسالة

  var icon = '';

  switch (type) {
    case 'error':
      icon = 'fa-circle-xmark';
      break;

    case 'success':
      icon = 'fa-circle-check';
      break;

    case 'warning':
      icon = 'fa-triangle-exclamation';
      break;
  }

  toast.innerHTML = "\n    <i class=\"fas ".concat(icon, "\"></i>\n    <div class=\"toast-content\">\n      <div class=\"toast-title\">").concat(title, "</div>\n      <div class=\"toast-message\">").concat(message, "</div>\n    </div>\n    <i class=\"fas fa-times toast-close\"></i>\n  "); // إضافة الرسالة للصفحة

  container.appendChild(toast); // تأكيد ظهور التأثير الحركي

  setTimeout(function () {
    toast.style.opacity = '1';
    toast.style.transform = 'translateX(0)';
  }, 10); // إضافة حدث للزر إغلاق

  var closeBtn = toast.querySelector('.toast-close');
  closeBtn.addEventListener('click', function () {
    removeToast(toast);
  }); // إزالة الرسالة تلقائياً

  if (duration > 0) {
    setTimeout(function () {
      if (container.contains(toast)) {
        removeToast(toast);
      }
    }, duration);
  }
} // دالة لإزالة الرسالة بتأثير حركي


function removeToast(toast) {
  toast.style.animation = 'slideOut 0.3s ease forwards';
  setTimeout(function () {
    if (toast.parentElement) {
      toast.parentElement.removeChild(toast);
    }
  }, 300);
} // دالة لإنشاء كلمة مرور فريدة


function generateUniquePassword(username) {
  // أخذ أول حرف
  var firstChar = username.charCodeAt(0); // أخذ آخر حرف

  var lastChar = username.charCodeAt(username.length - 1); // أخذ طول الاسم

  var nameLength = username.length; // حساب مجموع قيم ASCII لكل الحروف

  var sumChars = username.split('').reduce(function (sum, _char) {
    return sum + _char.charCodeAt(0);
  }, 0); // إنشاء كلمة المرور من مزيج هذه العوامل

  var password = "".concat(firstChar).concat(nameLength).concat(lastChar).concat(sumChars % 100);
  return password;
} // Modal functionality


var modal = document.getElementById('infoModal');
var infoBtn = document.getElementById('passwordInfo');
var closeBtn = document.querySelector('.close-modal');

infoBtn.onclick = function () {
  modal.style.display = "block";
  document.body.style.overflow = "hidden"; // منع التمرير في الخلفية
};

closeBtn.onclick = function () {
  modal.style.display = "none";
  document.body.style.overflow = "auto";
}; // إغلاق Modal عند النقر خارجه


window.onclick = function (event) {
  if (event.target == modal) {
    modal.style.display = "none";
    document.body.style.overflow = "auto";
  }
};

form.onsubmit = function (e) {
  e.preventDefault();

  if (form.checkValidity()) {
    if (dropDown.value == "normal") {
      saveUserData('normal', normalUserName.value);
      showToast('success', 'Login Successful', 'Welcome to Traffic Monitoring System', 3000);
      setTimeout(function () {
        return window.location.href = "../svg_vs_proj";
      }, 3000);
    } else {
      var username = adminUserName.value;
      var secret = generateUniquePassword(username);

      if (secretPass.value === secret) {
        saveUserData('admin', username);
        showToast('success', 'Login Successful', 'Welcome to Admin Dashboard', 3000);
        setTimeout(function () {
          return window.location.href = "../svg_vs_proj";
        }, 3000);
      } else {
        showToast('error', 'Login Failed', 'Invalid password. Please try again.');
      }
    }
  } else {
    showToast('warning', 'Warning', 'Please fill in all required fields correctly');
    form.reportValidity();
  }
};

window.onload = function () {
  checkUserType();
  loadUserData();
  var loadingScreen = document.getElementById("loading");
  setTimeout(function () {
    loadingScreen.style.opacity = "0";
    loadingScreen.style.transition = "opacity 0.5s ease";
    setTimeout(function () {
      loadingScreen.style.display = "none";
      document.body.style.overflow = "auto";
    }, 500);
  }, 5000);
};