...
form.onsubmit = (e) => {
  ...
  if (form.checkValidity()) {
    if (dropDown.value == "normal") {
      ...
      localStorage.setItem("currentUserType", "normal");
      showToast(
        "success",
        "Login Successful",
      ...
      setTimeout(() => {
        window.location.href = "../Choose_Real_OR_Sim/chooseDesign.html";
      }, 2000);
    } else {
      const adminPass = generateUniquePassword(adminUserName.value);
      if (secretPass.value === adminPass) {
        saveUserData("admin", adminUserName.value);
        // Store user type in localStorage
        localStorage.setItem("currentUserType", "admin");
        showToast(
          "success",
          "Login Successful",
          "Welcome Admin to Traffic Monitoring System",
          2000
        );
        setTimeout(() => {
          window.location.href = "../Choose_Real_OR_Sim/chooseDesign.html";
        ...
        showToast("error", "Login Failed", "Incorrect password!", 3000);
      }
    }
  }
};
